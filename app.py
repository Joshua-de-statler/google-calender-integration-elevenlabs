...
def get_availability():
    try:
        # ✅ FIX: Make the 'now' variable timezone-aware using UTC.
        utc = pytz.UTC
        ...
        free_slots = []
        # ✅ FIX: Make the 'check_time' variable timezone-aware.
        check_time = (start_of_search.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1)).replace(tzinfo=utc)
        
        while len(free_slots) < 5 and check_time < end_of_search:
            # Business hours: 9 AM to 5 PM SAST (UTC+2) -> 7:00 to 15:00 UTC
            if 7 <= check_time.hour < 15:
                is_free = True
        ...
        if not free_slots:
            return jsonify({"message": "Sorry, no slots are available in the next 7 days."})
...
def book_appointment():
    try:
        ...
        end_time_dt = start_time_dt + timedelta(minutes=15)

        event = {
            'summary': f'Onboarding Call with {data["name"]}',
            'location': 'Video Call - Link to follow',
            'description': f'A 15-minute onboarding call for {data["name"]}. Invitee email: {data["email"]}',
            'start': {
        ...
        success_message = (
            f"Perfect, {first_name}! I have successfully reserved that time slot on our calendar. "
